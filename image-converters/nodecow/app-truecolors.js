...
const outputFileName = `${path.parse(file).name}-true.cow`;
    ...
    function each(arr, func) {
      ...
      // add more variable names for full color support
      each('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ', l1 => {
        each('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', l2 => {
          // make sure the variable name always starts with a letter
          if (varNames.length < 61) {
            varNames.push(l1 + l2);
          } else {
            each('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', l3 => {
              // make sure the variable name always starts with a letter
              if (varNames.length < 3844) {
                varNames.push(l1 + l2 + l3);
              } else {
                each('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', l4 => {
                  // make sure the variable name always starts with a letter
                  if (varNames.length < 238328) {
                    varNames.push(l1 + l2 + l3 + l4);
                  }
                });
              }
            });
